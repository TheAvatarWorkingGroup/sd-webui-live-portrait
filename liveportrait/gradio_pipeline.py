...
class GradioPipeline(LivePortraitPipeline):
    ...
    def __init__(self, inference_cfg, crop_cfg, args: ArgumentConfig):
        ...
        self.args = args
        self.source_eye_ratio = 0.0
        self.source_lip_ratio = 0.0

    @torch.no_grad()
    def update_delta_new_eyeball_direction(
        self, eyeball_direction_x, eyeball_direction_y, delta_new, **kwargs
    ):
    ...
    @torch.no_grad()
    def update_delta_new_lip_variation_zero(
        self, lip_variation_zero, delta_new, **kwargs
    ):
    ...
    @torch.no_grad()
    def update_delta_new_lip_variation_one(
        self, lip_variation_one, delta_new, **kwargs
    ):
    ...
    @torch.no_grad()
    def update_delta_new_lip_variation_two(
        self, lip_variation_two, delta_new, **kwargs
    ):
    ...
    @torch.no_grad()
    def update_delta_new_lip_variation_three(
        self, lip_variation_three, delta_new, **kwargs
    ):
    ...
    @torch.no_grad()
    def update_delta_new_mov_z(self, mov_z, delta_new, **kwargs):
        delta_new[0, 5, 2] += mov_z - 1.0
    ...
    def execute_video(
        ...
        if input_source_path is not None and input_driving_path is not None:
            if (
                osp.exists(input_driving_path)
                and v_tab_selection == "Video"
                and not flag_crop_driving_video_input
                and is_square_video(input_driving_path) is False
            ):
                flag_crop_driving_video_input = True
                log(
                    "The driving video is not square, it will be cropped to square automatically."
                )
                gr.Info(
                    "The driving video is not square, it will be cropped to square automatically.",
                    duration=2,
            ...
            args_user = {
                ...
                "flag_camera_shake": flag_camera_shake,
                "camera_shake_amplitude": camera_shake_amplitude,
                "flag_inertial_lag": flag_inertial_lag,
                "inertial_lag_strength": inertial_lag_strength,
                "flag_global_transform": flag_global_transform,
                "global_transform_scale": global_transform_scale,
            }
            ...
            raise gr.Error(
                "Please upload the source portrait or source video, and driving video 🤗🤗🤗",
                duration=5,
            )
    ...
    def execute_image_retargeting(
        ...
        flag_animate_transition=False,
        flag_do_crop_input_retargeting_image=True,
    ):
        """for single image retargeting"""
        if (
            input_head_pitch_variation is None
            or input_head_yaw_variation is None
            or input_head_roll_variation is None
        ):
        ...
        (
            ...
            flag_do_crop=flag_do_crop_input_retargeting_image,
        )
        ...
        if input_eye_ratio is None or input_lip_ratio is None:
            ...
            if flag_animate_transition:
                # Generate frames for smooth transition
                n_frames = 30  # Number of frames for transition
                frames = []
                frames_blend = []

                # Calculate step sizes for each parameter
                mov_x_step = mov_x / n_frames
                mov_y_step = mov_y / n_frames
                mov_z_step = (mov_z - 1.0) / n_frames
                eyeball_x_step = eyeball_direction_x / n_frames
                eyeball_y_step = eyeball_direction_y / n_frames
                smile_step = smile / n_frames
                wink_step = wink / n_frames
                eyebrow_step = eyebrow / n_frames
                lip_zero_step = lip_variation_zero / n_frames
                lip_one_step = lip_variation_one / n_frames
                lip_two_step = lip_variation_two / n_frames
                lip_three_step = lip_variation_three / n_frames
                ...
                for i in track(
                    ...
                    # Interpolate values
                    current_mov_x = mov_x_step * i
                    current_mov_y = mov_y_step * i
                    current_mov_z = 1.0 + (mov_z_step * i)
                    current_eyeball_x = eyeball_x_step * i
                    current_eyeball_y = eyeball_y_step * i
                    current_smile = smile_step * i
                    current_wink = wink_step * i
                    current_eyebrow = eyebrow_step * i
                    current_lip_zero = lip_zero_step * i
                    current_lip_one = lip_one_step * i
                    current_lip_two = lip_two_step * i
                    current_lip_three = lip_three_step * i

                    # Convert to tensors
                    current_values = {
                        "mov_x": torch.tensor(current_mov_x).to(device),
                        "mov_y": torch.tensor(current_mov_y).to(device),
                        "mov_z": torch.tensor(current_mov_z).to(device),
                        "eyeball_direction_x": torch.tensor(current_eyeball_x).to(
                            device
                        ),
                        "eyeball_direction_y": torch.tensor(current_eyeball_y).to(
                            device
                        ),
                        "smile": torch.tensor(current_smile).to(device),
                        "wink": torch.tensor(current_wink).to(device),
                        "eyebrow": torch.tensor(current_eyebrow).to(device),
                        "lip_variation_zero": torch.tensor(current_lip_zero).to(device),
                        "lip_variation_one": torch.tensor(current_lip_one).to(device),
                        "lip_variation_two": torch.tensor(current_lip_two).to(device),
                        "lip_variation_three": torch.tensor(current_lip_three).to(
                            device
                        ),
                    }
                    ...
                    # Update deltas based on current interpolated values
                    delta_new = self.update_delta_new_mov_y(
                        current_values["mov_y"], delta_new
                    )
                    delta_new = self.update_delta_new_mov_x(
                        current_values["mov_x"], delta_new
                    )
                    delta_new = self.update_delta_new_mov_z(
                        current_values["mov_z"], delta_new
                    )
                    delta_new = self.update_delta_new_eyeball_direction(
                        current_values["eyeball_direction_x"],
                        current_values["eyeball_direction_y"],
                        delta_new,
                    )
                    delta_new = self.update_delta_new_smile(
                        current_values["smile"], delta_new
                    )
                    delta_new = self.update_delta_new_wink(
                        current_values["wink"], delta_new
                    )
                    delta_new = self.update_delta_new_eyebrow(
                        current_values["eyebrow"], delta_new
                    )
                    delta_new = self.update_delta_new_lip_variation_zero(
                        current_values["lip_variation_zero"], delta_new
                    )
                    delta_new = self.update_delta_new_lip_variation_one(
                        current_values["lip_variation_one"], delta_new
                    )
                    delta_new = self.update_delta_new_lip_variation_two(
                        current_values["lip_variation_two"], delta_new
                    )
                    delta_new = self.update_delta_new_lip_variation_three(
                        current_values["lip_variation_three"], delta_new
                    )
                    ...
                    if input_eye_ratio != self.source_eye_ratio:
                        combined_eye_ratio_tensor = (
                            self.live_portrait_wrapper.calc_combined_eye_ratio(
                                [[float(input_eye_ratio)]], source_lmk_user
                            )
                        )
                    ...
                    if input_lip_ratio != self.source_lip_ratio:
                        combined_lip_ratio_tensor = (
                            self.live_portrait_wrapper.calc_combined_lip_ratio(
                                [[float(input_lip_ratio)]], source_lmk_user
                            )
                        )
                    ...
                    if flag_stitching_retargeting_input:
                        x_d_new = self.live_portrait_wrapper.stitching(
                            x_s_user, x_d_new
                        )
                    out = self.live_portrait_wrapper.warp_decode(
                        f_s_user, x_s_user, x_d_new
                    )
                    ...
                    if flag_do_crop_input_retargeting_image:
                        out_to_ori_blend = paste_back(
                            out, crop_M_c2o, img_rgb, mask_ori
                        )
                    else:
                        out_to_ori_blend = out
                    frames_blend.append(out_to_ori_blend)

                # Create video from frames
                output_dir = self.args.output_dir
                mkdir(output_dir)
                video_path = osp.join(output_dir, "retargeting_animation.mp4")
                video_path_blend = osp.join(
                    output_dir, "retargeting_animation_blend.mp4"
                )

                log("Creating animation videos...")
                images2video(frames, wfp=video_path, fps=30)
                images2video(frames_blend, wfp=video_path_blend, fps=30)
                log("Animation generation complete!")

                return gr.update(value=video_path, visible=True), gr.update(
                    value=video_path_blend, visible=True
                )

            else:
                # Original single image retargeting logic
                ...
                delta_new = self.update_delta_new_mov_z(mov_z, delta_new)
                delta_new = self.update_delta_new_eyeball_direction(
    ...
    def prepare_retargeting_image(
        ...
        if input_image is not None:
            ...
            R_s_user = get_rotation_matrix(
                x_s_info["pitch"], x_s_info["yaw"], x_s_info["roll"]
            )
            R_d_user = get_rotation_matrix(
            ...
            raise gr.Error(
                "Please upload a source portrait as the retargeting input 🤗🤗🤗",
                duration=5,
            )
    ...
    def execute_video_retargeting(
        ...
        video_retargeting_silence=False,
        flag_do_crop_input_retargeting_video=True,
    ):
        ...
        if not video_retargeting_silence:
            (
                ...
                source_fps,
                n_frames,
            ) = self.prepare_retargeting_video(
                input_video,
                retargeting_source_scale,
                device,
                input_lip_ratio,
                driving_smooth_observation_variance_retargeting,
                flag_do_crop=flag_do_crop_input_retargeting_video,
            )
            if input_lip_ratio is None:
                ...
                I_p_lst = []
                for i in track(
                    range(n_frames), description="Retargeting video...", total=n_frames
                ):
                    ...
                    x_d_i_new = self.live_portrait_wrapper.stitching(
                        x_s_user_i, x_d_i_new
                    )
                    out = self.live_portrait_wrapper.warp_decode(
                        f_s_user_i, x_s_user_i, x_d_i_new
                    )
            ...
            (
                ...
                source_fps,
                n_frames,
            ) = self.prepare_video_lip_silence(
                input_video, device, flag_do_crop=flag_do_crop_input_retargeting_video
            )
            ...
            I_p_lst = []
            for i in track(
                range(n_frames), description="Silencing lip...", total=n_frames
            ):
                x_s_user_i = x_s_user_lst[i].to(device)
                f_s_user_i = f_s_user_lst[i].to(device)
                ...
                if flag_do_crop_input_retargeting_video:
                    I_p_pstbk = paste_back(
                        I_p_i, source_M_c2o_lst[i], source_rgb_lst[i], mask_ori_lst[i]
                    )
                    I_p_pstbk_lst.append(I_p_pstbk)
        ...
        frames_concatenated = concat_frames(
            ...
            source_image_lst=img_crop_256x256_lst,
            I_p_lst=I_p_lst,
        )
        wfp_concat = osp.join(
            self.args.output_dir, f"{basename(input_video)}_retargeting_concat.mp4"
        )
        images2video(frames_concatenated, wfp=wfp_concat, fps=source_fps)

        if flag_source_has_audio:
            # final result with concatenation
            wfp_concat_with_audio = osp.join(
                self.args.output_dir,
                f"{basename(input_video)}_retargeting_concat_with_audio.mp4",
            )
        ...
        if flag_source_has_audio:
            wfp_with_audio = osp.join(
                self.args.output_dir,
                f"{basename(input_video)}_retargeting_with_audio.mp4",
            )
    ...
    def prepare_retargeting_video(
        ...
        if input_video is not None:
            ...
            source_rgb_lst = [
                resize_to_limit(
                    img, inference_cfg.source_max_dim, inference_cfg.source_division
                )
            ...
            if flag_do_crop:
                ret_s = self.cropper.crop_source_video(
                    source_rgb_lst, self.cropper.crop_cfg
                )
                log(
                    f'Source video is cropped, {len(ret_s["frame_crop_lst"])} frames are processed.'
                )
                if len(ret_s["frame_crop_lst"]) != n_frames:
                    n_frames = min(len(source_rgb_lst), len(ret_s["frame_crop_lst"]))
                img_crop_256x256_lst, source_lmk_crop_lst, source_M_c2o_lst = (
                    ret_s["frame_crop_lst"],
                    ret_s["lmk_crop_lst"],
                    ret_s["M_c2o_lst"],
                )
                mask_ori_lst = [
                    prepare_paste_back(
                        inference_cfg.mask_crop,
                        source_M_c2o,
                        dsize=(source_rgb_lst[0].shape[1], source_rgb_lst[0].shape[0]),
                    )
                    for source_M_c2o in source_M_c2o_lst
                ]
            else:
                source_lmk_crop_lst = self.cropper.calc_lmks_from_cropped_video(
                    source_rgb_lst
                )
                img_crop_256x256_lst = [
                    cv2.resize(_, (256, 256)) for _ in source_rgb_lst
                ]  # force to resize to 256x256
                source_M_c2o_lst, mask_ori_lst = None, None

            c_s_eyes_lst, c_s_lip_lst = self.live_portrait_wrapper.calc_ratio(
                source_lmk_crop_lst
            )
            # save the motion template
            I_s_lst = self.live_portrait_wrapper.prepare_videos(img_crop_256x256_lst)
            source_template_dct = self.make_motion_template(
                I_s_lst, c_s_eyes_lst, c_s_lip_lst, output_fps=source_fps
            )
            ...
            for i in track(
                range(n_frames),
                description="Preparing retargeting video...",
                total=n_frames,
            ):
                ...
                combined_lip_ratio_tensor_retargeting = (
                    self.live_portrait_wrapper.calc_combined_lip_ratio(
                        c_d_lip_retargeting, source_lmk
                    )
                )
                lip_delta_retargeting = self.live_portrait_wrapper.retarget_lip(
                    x_s_user, combined_lip_ratio_tensor_retargeting
                )
                f_s_user_lst.append(f_s_user)
                x_s_user_lst.append(x_s_user)
                lip_delta_retargeting_lst.append(
                    lip_delta_retargeting.cpu().numpy().astype(np.float32)
                )
            lip_delta_retargeting_lst_smooth = smooth(
                lip_delta_retargeting_lst,
                lip_delta_retargeting_lst[0].shape,
                device,
                driving_smooth_observation_variance_retargeting,
            )
            ...
            raise gr.Error(
                "Please upload a source video as the retargeting input 🤗🤗🤗",
                duration=5,
            )
    ...
    def prepare_video_lip_silence(self, input_video, device, flag_do_crop=True):
        ...
        if input_video is not None:
            ...
            for i in track(
                range(n_frames),
                description="Preparing silencing lip...",
                total=n_frames,
            ):
                x_s_info = source_template_dct["motion"][i]
                x_s_info = dct2device(x_s_info, device)
                ...
                R_s = x_s_info["R"]
                t_s = x_s_info["t"]
                delta_new = torch.zeros_like(x_s_info["exp"]) + torch.from_numpy(
                    inference_cfg.lip_array
                ...
                f_s_user_lst.append(f_s_user)
                x_s_user_lst.append(x_s_user)
                x_d_i_new_lst.append(x_d_i_new)
            return (
                f_s_user_lst,
                x_s_user_lst,
                x_d_i_new_lst,
            ...
            # when press the clear button, go here
            raise gr.Error(
                "Please upload a source video as the input 🤗🤗🤗", duration=5
            )
...
class GradioPipelineAnimal(LivePortraitPipelineAnimal):
    ...
    def __init__(self, inference_cfg, crop_cfg, args: ArgumentConfig):
        inference_cfg.flag_crop_driving_video = (
            True  # ensure the face_analysis_wrapper is enabled
        )
        super().__init__(inference_cfg, crop_cfg)
    ...
    def execute_video(
        ...
        if input_source_path is not None and input_driving_path is not None:
            ...
            raise gr.Error(
                "Please upload the source animal image, and driving video 🤗🤗🤗",
                duration=5,
            )
