...
def init_live_portrait_pipeline(
    inference_cfg: InferenceConfig, crop_cfg: CropConfig, use_model_cache: bool
):
    global live_portrait_pipeline, live_portrait_pipeline_animal, retargeting_pipeline
    if not use_model_cache:
        clear_model_cache()
        return LivePortraitPipeline(inference_cfg=inference_cfg, crop_cfg=crop_cfg)
    if (
        not live_portrait_pipeline
        or live_portrait_pipeline.cropper.crop_cfg.model != crop_cfg.model
        or live_portrait_pipeline.cropper.crop_cfg.flag_force_cpu
        != crop_cfg.flag_force_cpu
        or live_portrait_pipeline.cropper.crop_cfg.device_id != crop_cfg.device_id
        or (
            crop_cfg.model == "facealignment"
            and (
                live_portrait_pipeline.cropper.crop_cfg.face_alignment_detector
                != crop_cfg.face_alignment_detector
                or live_portrait_pipeline.cropper.crop_cfg.face_alignment_detector_device
                != crop_cfg.face_alignment_detector_device
                or live_portrait_pipeline.cropper.crop_cfg.face_alignment_detector_dtype
                != crop_cfg.face_alignment_detector_dtype
            )
        )
    ):
        clear_model_cache()
        ...
        live_portrait_pipeline.live_portrait_wrapper.update_config(
            inference_cfg.__dict__
        )
    return live_portrait_pipeline


def init_live_portrait_animal_pipeline(
    inference_cfg: InferenceConfig, crop_cfg: CropConfig, use_model_cache: bool
):
    global live_portrait_pipeline, live_portrait_pipeline_animal, retargeting_pipeline
    if not use_model_cache:
    ...
    if (
        not live_portrait_pipeline_animal
        or live_portrait_pipeline_animal.cropper.crop_cfg.model != crop_cfg.model
        or live_portrait_pipeline_animal.cropper.crop_cfg.flag_force_cpu
        != crop_cfg.flag_force_cpu
        or live_portrait_pipeline_animal.cropper.crop_cfg.device_id
        != crop_cfg.device_id
        or (
            crop_cfg.model == "facealignment"
            and (
                live_portrait_pipeline_animal.cropper.crop_cfg.face_alignment_detector
                != crop_cfg.face_alignment_detector
                or live_portrait_pipeline_animal.cropper.crop_cfg.face_alignment_detector_device
                != crop_cfg.face_alignment_detector_device
                or live_portrait_pipeline_animal.cropper.crop_cfg.face_alignment_detector_dtype
                != crop_cfg.face_alignment_detector_dtype
            )
        )
    ):
        ...
        live_portrait_pipeline_animal.live_portrait_wrapper_animal.update_config(
            inference_cfg.__dict__
        )
    return live_portrait_pipeline_animal


def init_retargeting_pipeline(
    inference_cfg: InferenceConfig,
    crop_cfg: CropConfig,
    argument_cfg: ArgumentConfig,
    ...
    if (
        not retargeting_pipeline
        or retargeting_pipeline.cropper.crop_cfg.model != crop_cfg.model
        or retargeting_pipeline.cropper.crop_cfg.flag_force_cpu
        != crop_cfg.flag_force_cpu
        or retargeting_pipeline.cropper.crop_cfg.device_id != crop_cfg.device_id
        or (
            crop_cfg.model == "facealignment"
            and (
                retargeting_pipeline.cropper.crop_cfg.face_alignment_detector
                != crop_cfg.face_alignment_detector
                or retargeting_pipeline.cropper.crop_cfg.face_alignment_detector_device
                != crop_cfg.face_alignment_detector_device
                or retargeting_pipeline.cropper.crop_cfg.face_alignment_detector_dtype
                != crop_cfg.face_alignment_detector_dtype
            )
        )
    ):
...
def save_input_to_temp_file(input: str, input_extension: str, tmpdirname: str):
    temp_source_file = tempfile.NamedTemporaryFile(
        dir=tmpdirname, suffix=input_extension
    )
...
def is_file(input):
    return os.path.exists(input) or (
        type(input) is str
        and (input.startswith("http://") or input.startswith("https://"))
    )
...
def get_input_extension(input, input_default_extension=None):
    ...
    raise ValueError(
        "Invalid input extension. You must provide a file name with valid extension or fill in the source_file_extension or driving_file_extension parameter."
    )
...
def initialize_crop_model(
    crop_cfg: CropConfig,
    human_face_detector: Literal[
        None, "insightface", "mediapipe", "facealignment"
    ] = None,
    face_alignment_detector: Literal[
        None, "blazeface", "blazeface_back_camera", "retinaface", "sfd"
    ] = None,
    ...
    default_crop_model = cast(
        ...
        cast(
            str, opts.data.get("live_portrait_human_face_detector", "insightface")
        ).lower(),
    )
    ...
    default_face_alignment_detector = cast(
        ...
        cast(
            str,
            opts.data.get(
                "live_portrait_face_alignment_detector", "blazeface_back_camera"
            ),
        )
        .lower()
        .replace(" ", "_"),
    ...
    default_face_alignment_detector_device = cast(
        ...
        cast(
            str, opts.data.get("live_portrait_face_alignment_detector_device", "cuda")
        ).lower(),
    )
    ...
    default_face_alignment_detector_dtype = cast(
        ...
        cast(
            str, opts.data.get("live_portrait_face_alignment_detector_dtype", "fp16")
        ).lower(),
    )
    ...
    crop_cfg.face_alignment_detector = (
        ...
        if face_alignment_detector
        else default_face_alignment_detector
    )
    crop_cfg.face_alignment_detector_device = (
        face_alignment_detector_device
        if face_alignment_detector_device
        else default_face_alignment_detector_device
    )
    crop_cfg.face_alignment_detector_dtype = (
        face_alignment_detector_dtype
        if face_alignment_detector_dtype
        else default_face_alignment_detector_dtype
    )

    return crop_cfg


def rename_output_videos(
    wfp: str,
    wfp_concat: str,
    temp_output_dir: str,
    new_names_to_old_names: Dict[str, str],
):
...
def rename_output_videos_and_gif(
    wfp: str,
    wfp_concat: str,
    wfp_gif: str,
    temp_output_dir: str,
    ...
    wfp, wfp_concat = rename_output_videos(
        wfp, wfp_concat, temp_output_dir, new_names_to_old_names
    )
    ...
    with iio.imopen(wfp, "r") as wfp_f:
        for frame_index in frame_indices:
            if frame_index > 0:
                ...
                if save_output:
                    wfp_path = os.path.join(
                        os.path.dirname(wfp),
                        f"{basename(wfp)}_{frame_index}{source_file_extension}",
                    )
                    iio.imwrite(wfp_path, frame, extension=source_file_extension)
                if send_output:
                    animated_image = iio.imwrite(
                        "<bytes>", frame, extension=source_file_extension
                    )
    ...
    with iio.imopen(wfp_concat, "r") as wfp_concat_f:
        for frame_index in frame_indices:
            if frame_index > 0:
                ...
                if save_output:
                    wfp_concat_path = os.path.join(
                        os.path.dirname(wfp_concat),
                        f"{basename(wfp_concat)}_{frame_index}{source_file_extension}",
                    )
                    iio.imwrite(wfp_concat_path, frame, extension=source_file_extension)
                if send_output:
                    animated_image_with_concat = iio.imwrite(
                        "<bytes>", frame, extension=source_file_extension
                    )
                    animated_image_with_concat = (
                        base64.b64encode(animated_image_with_concat)
                    ).decode()
...
def save_videos_to_ouput(
    ...
    if send_output:
        ...
        with open(wfp_concat, "rb") as wfp_concat_f:
            animated_video_with_concat = (
                base64.b64encode(wfp_concat_f.read())
            ).decode()
    else:
        animated_video = None
...
def save_videos_and_gif_to_ouput(
    wfp: str,
    wfp_concat: str,
    wfp_gif: str,
    output_dir: str,
    ...
    animated_video, animated_video_with_concat = save_videos_to_ouput(
        wfp, wfp_concat, output_dir, temp_output_dir, save_output, send_output
    )
...
def live_portrait_api(_: gr.Blocks, app: FastAPI):
    ...
    class LivePortraitRequest(BaseModel):
        source: str = (
            ""  # path to the source portrait (human/animal) or video (human) or base64 encoded one
        )
        source_file_extension: str = (
            ".jpg"  # source file extension if source is a base64 encoded string or url
        )
        driving: str = (
            ""  # path to driving image, video or template (.pkl format) or base64 encoded one
        )
        driving_file_extension: str = (
            ".mp4"  # driving file extension if driving is a base64 encoded string or url
        )
        output_dir: str = "outputs/live-portrait/"  # directory to save output video
        output_mode: Literal["video", "images"] = (
            "video"  # whether to generate an output video or multiple images at given frame indices (see frame_indices)
        )
        frame_indices: List[int] = (
            []
        )  # frame indices where image snapshots should be generated (to be used with the 'images' output mode)
        ...
        flag_use_half_precision: bool = (
            True  # whether to use half precision (FP16). If black boxes appear, it might be due to GPU incompatibility; set to False.
        )
        flag_crop_driving_video: bool = (
            False  # whether to crop the driving video, if the given driving info is a video
        )
        device_id: int = 0  # gpu device id
        flag_force_cpu: bool = False  # force cpu inference, WIP!
        flag_normalize_lip: bool = (
            False  # whether to let the lip to close state before animation, only take effect when flag_eye_retargeting and flag_lip_retargeting is False
        )
        flag_source_video_eye_retargeting: bool = (
            False  # when the input is a source video, whether to let the eye-open scalar of each frame to be the same as the first source frame before the animation, only take effect when flag_eye_retargeting and flag_lip_retargeting is False, may cause the inter-frame jittering
        )
        flag_eye_retargeting: bool = (
            False  # not recommend to be True, WIP; whether to transfer the eyes-open ratio of each driving frame to the source image or the corresponding source frame
        )
        flag_lip_retargeting: bool = (
            False  # not recommend to be True, WIP; whether to transfer the lip-open ratio of each driving frame to the source image or the corresponding source frame
        )
        flag_stitching: bool = (
            True  # recommend to True if head movement is small, False if head movement is large or the source image is an animal
        )
        ...
        flag_do_crop: bool = (
            True  # whether to crop the source portrait or video to the face-cropping space
        )
        driving_option: Literal["expression-friendly", "pose-friendly"] = (
            "expression-friendly"  # "expression-friendly" or "pose-friendly"; "expression-friendly" would adapt the driving motion with the global multiplier, and could be used when the source is a human image
        )
        driving_multiplier: float = (
            1.0  # be used only when driving_option is "expression-friendly"
        )
        driving_smooth_observation_variance: float = (
            3e-7  # smooth strength scalar for the animated video when the input is a source video, the larger the number, the smoother the animated video; too much smoothness would result in loss of motion accuracy
        )
        audio_priority: Literal["source", "driving"] = (
            "driving"  # whether to use the audio from source or driving video
        )
        animation_region: Literal["exp", "pose", "lip", "eyes", "all"] = (
            "all"  # the region where the animation was performed, "exp" means the expression, "pose" means the head pose, "all" means all regions
        )
        ########## source crop arguments ##########
        det_thresh: float = 0.15  # detection threshold
        scale: float = 2.3  # the ratio of face area is smaller if scale is larger
        vx_ratio: float = (
            0  # the ratio to move the face to left or right in cropping space
        )
        vy_ratio: float = (
            -0.125
        )  # the ratio to move the face to up or down in cropping space
        flag_do_rot: bool = (
            True  # whether to conduct the rotation when flag_do_crop is True
        )
        source_max_dim: int = (
            1280  # the max dim of height and width of source image or video, you can change it to a larger number, e.g., 1920
        )
        source_division: int = (
            2  # make sure the height and width of source image or video can be divided by this number
        )
        ...
        human_face_detector: Literal[
            None, "insightface", "mediapipe", "facealignment"
        ] = None  # face detector to use for human inference ('insightface' by default)
        face_alignment_detector: Literal[
            None, "blazeface", "blazeface_back_camera", "retinaface", "sfd"
        ] = None
        face_alignment_detector_device: Literal[None, "cuda", "cpu", "mps"] = None
    ...
    def fast_check_inference_args(payload: LivePortraitRequest):
        ...
        if payload.output_mode == "images" and (
            payload.frame_indices is None or len(payload.frame_indices) == 0
        ):
            raise ValueError(
                "Frame indices are not optional when using 'images' output mode"
            )
    ...
    async def execute_human(payload: LivePortraitRequest = Body(...)) -> Any:
        ...
        with tempfile.TemporaryDirectory(dir=temp_dir) as tmpdirname:
            ...
            source_file_extension = get_input_extension(
                payload.source, payload.source_file_extension
            )
            driving_file_extension = get_input_extension(
                payload.driving, payload.driving_file_extension
            )

            argument_cfg.source = save_input_to_temp_file(
                payload.source, source_file_extension, tmpdirname
            )
            argument_cfg.driving = save_input_to_temp_file(
                payload.driving, driving_file_extension, tmpdirname
            )
            ...
            new_names_to_old_names = {
                source_tmp_name: (
                    basename(payload.source) if is_file(payload.source) else "source"
                ),
                driving_tmp_name: (
                    basename(payload.driving) if is_file(payload.driving) else "driving"
                ),
            }
            ...
            live_portrait_pipeline = init_live_portrait_pipeline(
                inference_cfg, crop_cfg, payload.use_model_cache
            )
            ...
            if payload.output_mode == "images":
                animated_images, animated_images_with_concat = (
                    save_video_frames_to_ouput(
                        wfp,
                        wfp_concat,
                ...
                return {
                    ...
                    "animated_images_with_concat": animated_images_with_concat,
                }
            ...
            return {
                ...
                "animated_video_with_concat": animated_video_with_concat,
            }
    ...
    class LivePortraitImageRetargetingRequest(LivePortraitCommonRetargetingRequest):
        ...
        retargeting_source_scale: float = (
            2.5  # the ratio of face area is smaller if scale is larger
        )
        flag_stitching_retargeting_input: bool = True  # To apply stitching or not
        flag_do_crop_input_retargeting_image: bool = (
    ...
    async def execute_image_retargeting(
        payload: LivePortraitImageRetargetingRequest = Body(...),
    ) -> Any:
        print(
            "Live Portrait API /live-portrait/human/retargeting/image received request"
        )
        ...
        with tempfile.TemporaryDirectory(dir=temp_dir) as tmpdirname:
            ...
            retargeting_pipeline = init_retargeting_pipeline(
                inference_cfg, crop_cfg, argument_cfg, payload.use_model_cache
            )
            ...
            for option_index in range(len(payload.retargeting_options)):
                ...
                suffix = (
                    "" if len(payload.retargeting_options) == 1 else f"_{option_index}"
                )
                ...
                wfp = os.path.join(
                    temp_output_dir,
                    f"{source_file_name}_retargeting{suffix}{payload.source_file_extension}",
                )
                wfp_concat = os.path.join(
                    temp_output_dir,
                    f"{source_file_name}_retargeting_cropped{suffix}{payload.source_file_extension}",
                )
            ...
            return {
                "retargeting_images": retargeting_images,
                "retargeting_images_cropped": retargeting_images_cropped,
            }
    ...
    async def init_image_retargeting(
        payload: LivePortraitImageRetargetingInitRequest = Body(...),
    ) -> Any:
        print(
            "Live Portrait API /live-portrait/human/retargeting/image/init received request"
        )
        ...
        with tempfile.TemporaryDirectory(dir=temp_dir) as tmpdirname:
            ...
            source_eye_ratio, source_lip_ratio = (
                retargeting_pipeline.init_retargeting_image(
                    payload.source_face_index,
                    payload.retargeting_source_scale,
                    payload.eye_ratio,
                    payload.lip_ratio,
            ...
            print(
                "Live Portrait API /live-portrait/human/retargeting/image/init finished"
            )
    ...
    class LivePortraitVideoRetargetingRequest(LivePortraitCommonRetargetingRequest):
        source_file_extension: str = (
            ".mp4"  # source file extension if source is a base64 encoded string or url
        )
        output_dir: str = "outputs/live-portrait/"  # directory to save output video
        send_output: bool = True
        save_output: bool = False
        ...
        lip_ratio: float = 0  # target lip-open ratio (0 -> 0.8)
        retargeting_source_scale: float = (
            2.3  # the ratio of face area is smaller if scale is larger
        )
        driving_smooth_observation_variance_retargeting: float = (
            3e-6  # motion smooth strength
        )
        video_retargeting_silence: bool = False  # keeping the lip silent
        flag_do_crop_input_retargeting_video: bool = (
            True  # whether to crop the source video to the face-cropping space
        )

    @app.post("/live-portrait/human/retargeting/video")
    async def execute_video_retargeting(
        payload: LivePortraitVideoRetargetingRequest = Body(...),
    ) -> Any:
        print(
            "Live Portrait API /live-portrait/human/retargeting/video received request"
        )
        ...
        with tempfile.TemporaryDirectory(dir=temp_dir) as tmpdirname:
            ...
            return {
                "retargeting_video": retargeting_video,
                "retargeting_video_with_concat": retargeting_video_with_concat,
            }
    ...
    async def execute_animal(payload: LivePortraitRequest = Body(...)) -> Any:
        ...
        if IS_MACOS:
            raise OSError(
                "XPose model, required to generate animal videos, is not compatible with MacOS systems."
            )
        ...
        with tempfile.TemporaryDirectory(dir=temp_dir) as tmpdirname:
            ...
            wfp, wfp_concat, wfp_gif = live_portrait_pipeline_animal.execute(
                argument_cfg
            )
            wfp, wfp_concat, wfp_gif = rename_output_videos_and_gif(
                wfp, wfp_concat, wfp_gif, temp_output_dir, new_names_to_old_names
            )
            ...
            animated_image, animated_image_with_concat, animated_gif = (
                save_videos_and_gif_to_ouput(
                    wfp,
                    wfp_concat,
                    wfp_gif,
                    payload.output_dir,
            ...
            print("Live Portrait API /live-portrait/animal finished")
            return {
                "animated_video": animated_image,
                "animated_video_with_concat": animated_image_with_concat,
            }
